 ...
 ******************************************************************************/
// TODO: Parameter adjustment required (sound buffer setting is currently ignored / adjustment required)
#define SAMPLE_COUNT PSP_AUDIO_SAMPLE_ALIGN(256) // The number of samples
//#define SAMPLE_COUNT        256
#define SAMPLE_SIZE (SAMPLE_COUNT * 2) // Number of buffers per sample
#define GBC_NOISE_WRAP_FULL 32767
#define GBC_NOISE_WRAP_HALF 126

#define RENDER_SAMPLE_NULL()

// Write LEFT CHANNEL data to the sound buffer
...
// Write RIGHT CHANNEL data to the sound buffer
#define RENDER_SAMPLE_RIGHT()                                \
    sound_buffer[sound_write_offset + 1] += current_sample + \
                                            FP16_16_TO_U32((next_sample - current_sample) * fifo_fractional)

// Write LEFT / RIGHT CHANNEL data to the sound buffer
...
#define GET_NOISE_SAMPLE_FULL() \
    current_sample =            \
        ((s32)(noise_table15[FP16_16_TO_U32(sample_index) >> 5] << (FP16_16_TO_U32(sample_index) & 0x1F)) >> 31) & 0x0F

#define GET_NOISE_SAMPLE_HALF() \
    current_sample =            \
        ((s32)(noise_table7[FP16_16_TO_U32(sample_index) >> 5] << (FP16_16_TO_U32(sample_index) & 0x1F)) >> 31) & 0x0F
...
static s16 sound_buffer[BUFFER_SIZE]; // Sound buffer 2n = Left / 2n + 1 = Right
static u32 sound_read_offset = 0;     // Sound buffer read offset
static SceUID sound_thread;
static u32 sound_last_cpu_ticks = 0;
static FIXED16_16 gbc_sound_tick_step;
volatile static u32 audio_thread_exit_flag; // End flag for audio thread.
...
void sound_timer(FIXED16_16 frequency_step, u32 channel)
{
    ...
    // Improved slowdowns on magical vacations
    u8 fifo_length;
...
static int sound_update_thread(SceSize args, void *argp)
{
    ...
    while (!audio_thread_exit_flag)
    {
        ...
        {
            // Which is faster, TODO memcopy * 2?
        ...
        else
        {
            if (pause_sound_flag == 1)
                buffer_num = 0; // Play all 0 data
            else
                buffer_num = 1; // Play the previous data as it is
        }
...
// Stop the emulator until the sound data drops below a certain level
void synchronize_sound()
{
    while (CHECK_BUFFER() >= audio_buffer_size) // TODO: Adjustment required
