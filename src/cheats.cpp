...
static void decrypt_gsa_code(int *address_ptr, int *value_ptr, cheat_variant_t cheat_variant)
{
    ...
    //Use v1 or v3 seeds depending on variant passed
    u32 *seeds;
...
void add_cheats(char *cheats_filename)
{
    ...
    //Get cheat_path
    //If a default cheat dir is set, use it along the filename
    ...
    if (cheats_file)
    {
        while (fgets(current_line, 256, cheats_file))
        {
            ...
            //Gameshark v1,v2 and PAR v1,v2 are all GAMESHARK_V1 types
            ...
            //If we got a valid match
            if (current_cheat_variant != CHEAT_TYPE_INVALID)
            {
                //Copy name to game config
                ...
                //Remove also trailing \r if name ended with \r\n
                if (cheat_name_length &&
                    g_game_config.cheats_flag[g_cheats_num].cheat_name[cheat_name_length - 1] == '\r')
                    g_game_config.cheats_flag[g_cheats_num].cheat_name[cheat_name_length - 1] = 0;
                ...
                //Now time to read the cheat code itself
                while (fgets(current_line, 256, cheats_file))
                {
                    ...
                    //Decrypt gameshark code if necessary
                ...
                //Add number of lines of this cheat
                g_game_config.cheats_flag[g_cheats_num].num_cheat_lines = num_cheat_lines;

                //Increase number of cheats
                g_cheats_num++;
                //Stop if reached max number of cheats
                if (g_cheats_num == MAX_CHEATS)
                    break;
            }
        }
        fclose(cheats_file);
    ...
    for (i = 0; i < cheat->num_cheat_lines; i++)
    {
        ...
        case 0x6:
            if (gamepak_file_large == -1)                                              // Only for on-memory ROM
                ADDRESS16(gamepak_rom, (address * 2) - 0x08000000) = (value & 0xFFFF); // Writing data
            break;
