...
void render_scanline_conditional_tile(u32 start, u32 end, u16 *scanline,
                                      u32 enable_flags, u32 dispcnt, u32 bldcnt, tile_layer_render_struct *layer_renderers);

void render_scanline_conditional_bitmap(u32 start, u32 end, u16 *scanline,
                                        u32 enable_flags, u32 dispcnt, u32 bldcnt, bitmap_layer_render_struct *layer_renderers);
...
// Set layer_renderers from VIDEO MODE
...
// TODO speedup
// Compared with the contents of the previous register, if there is no change, it may be possible to speed up by skipping the process
// Sort the objects according to their priority.
void order_obj(u32 video_mode)
{
    s32 obj_num, row;
    s32 obj_x, obj_y;
    ...
    for (obj_num = 127; obj_num >= 0; obj_num--, oam_ptr -= 4)
    {
        ...
        if (((obj_attribute_0 & 0x0300) != 0x0200) && (obj_size != 0xC000) && (obj_mode != 3) && ((video_mode < 3) || ((obj_attribute_2 & 0x3FF) >= 512)))
        {
            obj_y = obj_attribute_0 & 0xFF;
            if (obj_y > 160)
                obj_y -= 256;
...
// VIDEO MODE 3,4,5 / Drawing in non-window mode
void render_scanline_bitmap(u16 *scanline, u32 dispcnt)
{
    // Set layer_renderers from VIDEO MODE
...
{
    u32 pitch = screen_pitch;                               // Framebuffer width (240/512)
    u32 dispcnt = io_registers[REG_DISPCNT];                // IO DISPCNT
    u32 vcount = io_registers[REG_VCOUNT];                  // IO VCOUNT Current line value (0 ~ 277)
    u16 *screen_offset = screen_address + (vcount * pitch); // Address of the leftmost pixel
    u32 video_mode = dispcnt & 0x07;                        // VIDEO MODE(0~5)

    // Sort objects when OAM changes
    if (oam_update)
    {
    ...
    if (dispcnt & 0x80)
    {
        // White line is displayed in forced blank mode
