...
// gpSP configuration file header
// Header up to 4 bytes
...
// GAME configuration file header
// Header up to 4 bytes
...
#define ANALOG_CONFIG_OPTION(display_string, number)                                \
    {                                                                               \
        NULL,                                                                       \
            &menu::menu_fix_analog_help,                                            \
            NULL,                                                                   \
            display_string,                                                         \
            gamepad_config_buttons,                                                 \
            gamepad_config_map + analog_config_line_to_button[number],              \
            sizeof(gamepad_config_buttons) / sizeof(gamepad_config_buttons[0]),     \
            gamepad_help[gamepad_config_map[analog_config_line_to_button[number]]], \
            number,                                                                 \
            STRING_SELECTION_TYPE                                                   \
    }
...
#define ACTION_SELECTION_OPTION(action_function, passive_function,                                          \
    ...
    {                                                                                                       \
        action_function,                                                                                    \
            ...
            help_string,                                                                                    \
            line_number,                                                                                    \
            (MENU_OPTION_TYPE_ENUM)(type | ACTION_TYPE)                                                     \
    }
...
s32 load_file(char **wildcards, char *result, char *default_dir_name)
{
    ...
    char **file_list;
    char **dir_list;
    u32 num_files; // Number of files in the current directory
    u32 num_dirs;  // Number of folders in the current directory
    char *file_name;
    u32 file_name_length;
    s32 ext_pos = -1;
    u32 chosen_file, chosen_dir;
    s32 return_value = 1;
    u32 current_file_selection;    // Selected file
    u32 current_file_scroll_value; // Scroll position
...
menu::menu() : repeat(1),
               return_value(0),
               first_load(0),
               gamepad_help({msg[MSG_PAD_MENU_CFG_HELP_0],
               ...
               yes_no_options({msg[MSG_NO],
                               msg[MSG_YES]}),
               enable_disable_options({msg[MSG_DISABLED],
                                       msg[MSG_ENABLED]}),
               scale_options({msg[MSG_SCALE_UNSCALED],
                              msg[MSG_SCALE_SCALED],
                              msg[MSG_SCALE_FULL],
                              msg[MSG_SCN_OPT1],
                              msg[MSG_SCN_OPT2]}),
               frameskip_options({msg[MSG_FS_AUTO], msg[MSG_FS_MANUAL], msg[MSG_FS_OFF]}),
               frameskip_variation_options({msg[MSG_FS_UNIFORM], msg[MSG_FS_RANDOM]}),
               audio_buffer_options({msg[MSG_SB_2048], msg[MSG_SB_3072], msg[MSG_SB_4096], msg[MSG_SB_5120], msg[MSG_SB_6144],
                                     msg[MSG_SB_7168], msg[MSG_SB_8192], msg[MSG_SB_9216], msg[MSG_SB_10240], msg[MSG_SB_11264], msg[MSG_SB_12288]}),
               update_backup_options({msg[MSG_BK_EXITONLY], msg[MSG_BK_AUTO]}),
               clock_speed_options({msg[MSG_CLK_33], msg[MSG_CLK_66], msg[MSG_CLK_100], msg[MSG_CLK_133], msg[MSG_CLK_166],
                                    msg[MSG_CLK_200], msg[MSG_CLK_233], msg[MSG_CLK_266], msg[MSG_CLK_300], msg[MSG_CLK_333]}),
               gamepad_config_buttons({msg[MSG_PAD_MENU_CFG_0],
               ...
               ratio_options({msg[MSG_R_4_3], msg[MSG_R_16_9]}),
               interlace_options({msg[MSG_I_NON], msg[MSG_I_ON]}),
               language_options({lang[0],
                                 lang[1],
                                 lang[2],
                                 lang[3],
                                 lang[4],
                                 ...
                                 lang[10],
                                 lang[11]}),
               boot_mode_options({"GAME CART",
                                  "GBA BIOS"})
#ifdef USE_C_CORE
               ,
               emulate_core_options[] =
                   {
                       "ASM CORE",
                       "C CORE"};
#endif
...
//Overload () operator
u32 menu::operator()(u16 *original_screen)
{
    /*--------------------------------------------------------
     Graphic sound options
  --------------------------------------------------------*/
    graphics_sound_options[0] = STRING_SELECTION_OPTION(NULL, msg[MSG_SCALE], scale_options, &g_gpsp_config.screen_mode, 5, msg[MSG_G_S_MENU_HELP_0], 0);
    graphics_sound_options[1] = STRING_SELECTION_OPTION(NULL, msg[MSG_FILTER], yes_no_options, &g_gpsp_config.screen_filter, 2, msg[MSG_G_S_MENU_HELP_1], 1);
    graphics_sound_options[2] = STRING_SELECTION_OPTION(NULL, msg[MSG_RATIO], ratio_options, &g_gpsp_config.screen_ratio, 2, msg[MSG_G_S_MENU_HELP_9], 2);
    graphics_sound_options[3] = STRING_SELECTION_OPTION(NULL, msg[MSG_INTERLACE], interlace_options, &g_gpsp_config.screen_interlace, 2, msg[MSG_G_S_MENU_HELP_11], 3);
    graphics_sound_options[4] = STRING_SELECTION_OPTION(NULL, msg[MSG_SKIP_TYPE], frameskip_options, &g_game_config.frameskip_type, 3, msg[MSG_G_S_MENU_HELP_2], 5);
    graphics_sound_options[5] = NUMERIC_SELECTION_OPTION(NULL, msg[MSG_SKIP_VALUE], &g_game_config.frameskip_value, 100, msg[MSG_G_S_MENU_HELP_3], 6);
    graphics_sound_options[6] = STRING_SELECTION_OPTION(NULL, msg[MSG_SKIP_RANDOM], frameskip_variation_options, &g_game_config.random_skip, 2, msg[MSG_G_S_MENU_HELP_4], 7);
    graphics_sound_options[7] = STRING_SELECTION_OPTION(NULL, msg[MSG_AUDIO_ENABLE], yes_no_options, &g_gpsp_config.enable_audio, 2, msg[MSG_G_S_MENU_HELP_5], 9);
    graphics_sound_options[8] = STRING_SELECTION_OPTION(NULL, msg[MSG_AUDIO_BUFFER], audio_buffer_options, &g_game_config.audio_buffer_size_number, 11, msg[MSG_G_S_MENU_HELP_6], 10);
    graphics_sound_options[9] = (MENU_OPTION_TYPE)ACTION_OPTION(&menu::menu_save_ss, NULL, msg[MSG_SCREEN_SHOT], msg[MSG_G_S_MENU_HELP_7], 12);
    graphics_sound_options[10] = NUMERIC_SELECTION_OPTION(NULL, msg[MSG_DISPLAY_SCALE], &g_gpsp_config.screen_scale, 201, msg[MSG_G_S_MENU_HELP_10], 14);
    graphics_sound_options[11] = SUBMENU_OPTION(NULL, msg[MSG_MENU_RETURN_MAIN], msg[MSG_G_S_MENU_HELP_8], 16);

    MAKE_MENU(graphics_sound, &menu::submenu_graphics_sound, NULL);

    /*--------------------------------------------------------
     Cheat options
  --------------------------------------------------------*/
    for (u8 i = 0; i < 10; i++)
        cheats_options[i] = CHEAT_OPTION((10 * menu_cheat_page) + i);
    cheats_options[10] = (MENU_OPTION_TYPE)NUMERIC_SELECTION_OPTION(&menu::reload_cheats_page, msg[MSG_CHEAT_MENU_5], &menu_cheat_page, MAX_CHEATS_PAGE, msg[MSG_CHEAT_MENU_HELP_5], 11);
    cheats_options[11] = (MENU_OPTION_TYPE)ACTION_OPTION(&menu::menu_load_cheat_file, NULL, msg[MSG_CHEAT_MENU_1], msg[MSG_CHEAT_MENU_HELP_1], 13);
    cheats_options[12] = SUBMENU_OPTION(NULL, msg[MSG_CHEAT_MENU_4], msg[MSG_CHEAT_MENU_HELP_4], 16);

    MAKE_MENU(cheats, &menu::submenu_cheats, NULL);
  ...
  --------------------------------------------------------*/
    MENU_OPTION_TYPE misc_options[] =
    {
        ...
        STRING_SELECTION_OPTION(&menu::home_mode, msg[MSG_CHEAT_MENU_6], yes_no_options, &g_gpsp_config.enable_home, 2, msg[MSG_CHEAT_MENU_HELP_6], 2),
    ...
    MAKE_MENU(misc, &menu::submenu_misc, NULL);

    /*--------------------------------------------------------
     Save state option
  --------------------------------------------------------*/
    savestate_options[0] = (MENU_OPTION_TYPE)NUMERIC_SELECTION_HIDE_OPTION(&menu::menu_load_state, &menu::menu_change_state, msg[MSG_STATE_MENU_0], &g_savestate_slot_num, 11, msg[MSG_STATE_MENU_HELP_0], 6);
    savestate_options[1] = (MENU_OPTION_TYPE)NUMERIC_SELECTION_HIDE_OPTION(&menu::menu_save_state, &menu::menu_change_state, msg[MSG_STATE_MENU_1], &g_savestate_slot_num, 11, msg[MSG_STATE_MENU_HELP_1], 7);
    savestate_options[2] = (MENU_OPTION_TYPE)NUMERIC_SELECTION_HIDE_OPTION(&menu::menu_load_state_file, &menu::menu_change_state, msg[MSG_STATE_MENU_2], &g_savestate_slot_num, 11, msg[MSG_STATE_MENU_HELP_2], 9);
    savestate_options[3] = (MENU_OPTION_TYPE)NUMERIC_SELECTION_OPTION(&menu::menu_change_state, msg[MSG_STATE_MENU_3], &g_savestate_slot_num, 11, msg[MSG_STATE_MENU_HELP_3], 11);
    savestate_options[4] = SUBMENU_OPTION(NULL, msg[MSG_STATE_MENU_4], msg[MSG_STATE_MENU_HELP_4], 15);

    MAKE_MENU(savestate, &menu::submenu_savestate, NULL);

    /*--------------------------------------------------------
     Gamepad options
  --------------------------------------------------------*/
    for (u8 i = 0; i < 12; i++)
        gamepad_config_options[i] = (MENU_OPTION_TYPE)GAMEPAD_CONFIG_OPTION(msg[MSG_PAD_MENU_0 + i], i);
    gamepad_config_options[12] = (MENU_OPTION_TYPE)STRING_SELECTION_OPTION(&menu::set_gamepad, msg[MSG_PAD_MENU_12], yes_no_options, &g_game_config.use_default_gamepad_map, 2, msg[MSG_PAD_MENU_HELP_12], 13);
    gamepad_config_options[13] = SUBMENU_OPTION(NULL, msg[MSG_PAD_MENU_13], msg[MSG_PAD_MENU_HELP_13], 15);

    MAKE_MENU(gamepad_config, &menu::submenu_gamepad, NULL);

    /*--------------------------------------------------------
     Analog pad option
  --------------------------------------------------------*/
    for (u8 i = 0; i < 4; i++)
        analog_config_options[i] = (MENU_OPTION_TYPE)ANALOG_CONFIG_OPTION(msg[MSG_A_PAD_MENU_0 + i], i);
    analog_config_options[4] = STRING_SELECTION_OPTION(NULL, msg[MSG_A_PAD_MENU_4], yes_no_options, &g_gpsp_config.enable_analog, 2, msg[MSG_A_PAD_MENU_HELP_0], 7);
    analog_config_options[5] = NUMERIC_SELECTION_OPTION(NULL, msg[MSG_A_PAD_MENU_5], &g_gpsp_config.analog_sensitivity_level, 10, msg[MSG_A_PAD_MENU_HELP_1], 8);
    analog_config_options[6] = STRING_SELECTION_OPTION(NULL, msg[MSG_A_PAD_MENU_6], yes_no_options, &g_game_config.allocate_sensor, 2, msg[MSG_A_PAD_MENU_HELP_2], 9); /* TODO */
    analog_config_options[7] = STRING_SELECTION_OPTION(&menu::set_gamepad, msg[MSG_PAD_MENU_12], yes_no_options, &g_game_config.use_default_gamepad_map, 2, msg[MSG_PAD_MENU_HELP_12], 11);
    analog_config_options[8] = SUBMENU_OPTION(NULL, msg[MSG_A_PAD_MENU_7], msg[MSG_A_PAD_MENU_HELP_3], 15);

    MAKE_MENU(analog_config, &menu::submenu_analog, NULL);
  ...
  --------------------------------------------------------*/
    MENU_OPTION_TYPE main_options[] =
        {
            ACTION_OPTION(&menu::menu_load, NULL, msg[MSG_MAIN_MENU_7], msg[MSG_MAIN_MENU_HELP_7], 0),    /* 11行目 */
            ACTION_OPTION(&menu::menu_exit, NULL, msg[MSG_MAIN_MENU_9], msg[MSG_MAIN_MENU_HELP_9], 1),    /* 13行目 */
            ACTION_OPTION(&menu::menu_restart, NULL, msg[MSG_MAIN_MENU_8], msg[MSG_MAIN_MENU_HELP_8], 2), /* 12行目 */

            NUMERIC_SELECTION_ACTION_OPTION(&menu::menu_load_state, NULL, msg[MSG_MAIN_MENU_1], &g_savestate_slot_num, 11, msg[MSG_MAIN_MENU_HELP_1], 4), /*  2行目 */
            NUMERIC_SELECTION_ACTION_OPTION(&menu::menu_save_state, NULL, msg[MSG_MAIN_MENU_2], &g_savestate_slot_num, 11, msg[MSG_MAIN_MENU_HELP_2], 5), /*  3行目 */
            ...
            ACTION_OPTION(&menu::menu_quit, NULL, msg[MSG_MAIN_MENU_10], msg[MSG_MAIN_MENU_HELP_10], 16) /* 15行目 */
        };

    MAKE_MENU(main, &menu::submenu_main, NULL);

    // Menu processing from here
    ...
    choose_menu(&main_menu, &main_menu, original_screen);
    ...
    reload_cheats_page(&main_menu, original_screen);

    (this->*current_menu->init_function)(&main_menu, original_screen);
    ...
    while (repeat)
    {
                                               ...
                                               STRING_SELECTION_TYPE))
            {
                *(current_option->current_option) =
                ...
                if (current_option->passive_function)
                    (this->*current_option->passive_function)(&main_menu, original_screen);
            }
            break;
        ...
        case CURSOR_EXIT:
            if (current_menu == &main_menu)
                menu_exit(&main_menu, original_screen);
        ...
        case CURSOR_SELECT:
            if (current_option->option_type & ACTION_TYPE)
                (this->*current_option->action_function)(&main_menu, original_screen);

            else if ((current_option->option_type & SUBMENU_TYPE) && current_option->sub_menu)
                choose_menu(current_option->sub_menu, &main_menu, original_screen);
...
void menu::choose_menu(MENU_TYPE *new_menu, MENU_TYPE *main_menu, u16 *original_screen)
{
    if (new_menu == NULL)
        new_menu = main_menu;
...
void menu::menu_exit(MENU_TYPE *main_menu, u16 *original_screen)
{
    if (!first_load)
        repeat = 0;
}

void menu::menu_quit(MENU_TYPE *main_menu, u16 *original_screen)
{
    quit(0);
}
...
void menu::menu_load(MENU_TYPE *main_menu, u16 *original_screen)
{
    ...
    //   if (! g_gpsp_config.update_backup_flag) // Consider TODO timing
    update_backup_force();
    ...
    {
        choose_menu(current_menu, main_menu, original_screen);
    }
}

void menu::menu_restart(MENU_TYPE *main_menu, u16 *original_screen)
{
    if (!first_load)
    {
        reset_gba();
...
void menu::menu_save_ss(MENU_TYPE *main_menu, u16 *original_screen)
{
    if (!first_load)
        save_ss_bmp(original_screen);
}

void menu::menu_change_state(MENU_TYPE *main_menu, u16 *original_screen)
{
    get_savestate_filename(g_savestate_slot_num, current_savestate_filename);
}

void menu::menu_save_state(MENU_TYPE *main_menu, u16 *original_screen)
{
    menu_change_state(main_menu, original_screen);
    if (!first_load)
    {
...
void menu::menu_load_state(MENU_TYPE *main_menu, u16 *original_screen)
{
    menu_change_state(main_menu, original_screen);
    if (!first_load)
    {
...
void menu::menu_fix_gamepad_help(MENU_TYPE *main_menu, u16 *original_screen)
{
    current_option->help_string =
        gamepad_help[gamepad_config_map[gamepad_config_line_to_button[current_option_num]]];
}

void menu::menu_fix_analog_help(MENU_TYPE *main_menu, u16 *original_screen)
{
    current_option->help_string =
        gamepad_help[gamepad_config_map[analog_config_line_to_button[current_option_num]]];
}

void menu::submenu_graphics_sound(MENU_TYPE *main_menu, u16 *original_screen)
{
}

void menu::submenu_cheats(MENU_TYPE *main_menu, u16 *original_screen)
{
}

void menu::submenu_misc(MENU_TYPE *main_menu, u16 *original_screen)
{
}

void menu::submenu_gamepad(MENU_TYPE *main_menu, u16 *original_screen)
{
}

#ifdef USE_ADHOC
void menu::submenu_adhoc(MENU_TYPE *main_menu, u16 *original_screen)
{
}

void menu::adhoc_connect_menu(MENU_TYPE *main_menu, u16 *original_screen)
{
    set_cpu_clock(g_game_config.clock_speed_number);
...
void menu::adhoc_disconnect_menu(MENU_TYPE *main_menu, u16 *original_screen)
{
    adhoc_exit();
}
...
void menu::submenu_analog(MENU_TYPE *main_menu, u16 *original_screen)
{
}

void menu::submenu_savestate(MENU_TYPE *main_menu, u16 *original_screen)
{
    PRINT_STRING_BG(msg[MSG_STATE_MENU_TITLE], COLOR_HELP_TEXT, COLOR_BG, 10, 70);
    menu_change_state(main_menu, original_screen);
}

void menu::submenu_main(MENU_TYPE *main_menu, u16 *original_screen)
{
    get_savestate_filename_noshot(g_savestate_slot_num, current_savestate_filename);
}

void menu::reload_cheats_page(MENU_TYPE *main_menu, u16 *original_screen)
{
    for (i = 0; i < 10; i++)
    {
...
void menu::home_mode(MENU_TYPE *main_menu, u16 *original_screen)
{
    sceImposeSetHomePopup(g_gpsp_config.enable_home);
}

void menu::set_gamepad(MENU_TYPE *main_menu, u16 *original_screen)
{
...
u32 load_dircfg(char *file_name) // TODO: Rewrite to smart implementation
{
    int loop;
    int next_line;
...
u32 load_fontcfg(char *file_name) // TODO: Rewrite to smart implementation
{
    int loop;
    int next_line;
    char current_line[256];
        ...
        while (fgets(current_line, 256, msg_file))
        {
            if (parse_line(current_line, current_str) != -1)
            {
                if (loop <= (MSG_END + 1 + next_line))
                {
                    ...
                    {
                        // For new lines
                        msg[loop] = &msg_data[offset]; // New line
                        next_line = 1;
                        loop++;
                    }
                    strcpy(&msg_data[offset], current_str);
                    offset = offset + strlen(current_str); // offset indicates the position of NULL
                }
            }
            else
            {
...
static void print_status(u32 mode)
{
    ...
    sprintf(print_buffer_1, "MAX ROM BUF: %02d MB Ver: %d.%d %s %d",
