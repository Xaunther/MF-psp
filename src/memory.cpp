...
// TODO: PSP-1000 uses VRAM because the frame buffer needs to be 256KB.
...
// GBA ROM / RAM total 962kb
// Palette RAM 1 kb
u16 palette_ram[512];
// Object attribute RAM 1kb
u16 oam_ram[512];
// IO register 32kb
u16 io_registers[1024 * 16];
// Ext work RAM 256KB x2
u8 ewram[1024 * 256 * 2];
// Int work RAM 32KB x2
u8 iwram[1024 * 32 * 2];
// VRAM 192kb
u8 vram[1024 * 96 * 2];
// BIOS ROM 32kb
...
#define waitstate_control()                                                                                                                              \
    {                                                                                                                                                    \
        ...
        u8 waitstate_table[4] = {5, 4, 3, 9};                                                                                                            \
                                                                                                                                                         \
        waitstate_cycles_non_seq[0][0x0e] = waitstate_cycles_seq[0][0x0e] = waitstate_table[value & 0x03];                                               \
                                                                                                                                                         \
        for (i = 0; i < 2; i++)                                                                                                                          \
        {                                                                                                                                                \
            waitstate_cycles_seq[i][0x08] = waitstate_cycles_seq[i][0x09] = gamepak_waitstate_seq[(value >> 4) & 0x01][i][0];                            \
            waitstate_cycles_seq[i][0x0A] = waitstate_cycles_seq[i][0x0B] = gamepak_waitstate_seq[(value >> 7) & 0x01][i][1];                            \
            waitstate_cycles_seq[i][0x0C] = waitstate_cycles_seq[i][0x0D] = gamepak_waitstate_seq[(value >> 10) & 0x01][i][2];                           \
        }                                                                                                                                                \
                                                                                                                                                         \
        waitstate_cycles_non_seq[0][0x08] = waitstate_cycles_non_seq[0][0x09] = waitstate_table[(value >> 2) & 0x03];                                    \
        waitstate_cycles_non_seq[0][0x0A] = waitstate_cycles_non_seq[0][0x0B] = waitstate_table[(value >> 5) & 0x03];                                    \
        waitstate_cycles_non_seq[0][0x0C] = waitstate_cycles_non_seq[0][0x0D] = waitstate_table[(value >> 8) & 0x03];                                    \
                                                                                                                                                         \
        /* 32bit access ( split into two 16bit accsess ) */                                                                                              \
        waitstate_cycles_non_seq[1][0x08] = waitstate_cycles_non_seq[1][0x09] = (waitstate_cycles_non_seq[0][0x08] + waitstate_cycles_seq[0][0x08] - 1); \
        waitstate_cycles_non_seq[1][0x0A] = waitstate_cycles_non_seq[1][0x0B] = (waitstate_cycles_non_seq[0][0x0A] + waitstate_cycles_seq[0][0x0A] - 1); \
        waitstate_cycles_non_seq[1][0x0C] = waitstate_cycles_non_seq[1][0x0D] = (waitstate_cycles_non_seq[0][0x0C] + waitstate_cycles_seq[0][0x0C] - 1); \
                                                                                                                                                         \
        /* gamepak prefetch */                                                                                                                           \
        if (value & 0x4000)                                                                                                                              \
        {                                                                                                                                                \
        ...
        {                                                                                                                                                \
            for (i = 0; i < 2; i++)                                                                                                                      \
            {                                                                                                                                            \
                cpu_waitstate_cycles_seq[i][0x08] = cpu_waitstate_cycles_seq[i][0x09] = waitstate_cycles_seq[i][0x08];                                   \
                cpu_waitstate_cycles_seq[i][0x0A] = cpu_waitstate_cycles_seq[i][0x0B] = waitstate_cycles_seq[i][0x0A];                                   \
                cpu_waitstate_cycles_seq[i][0x0C] = cpu_waitstate_cycles_seq[i][0x0D] = waitstate_cycles_seq[i][0x0C];                                   \
            }                                                                                                                                            \
        }                                                                                                                                                \
                                                                                                                                                         \
...
CPU_ALERT_TYPE write_io_register32(u32 address, u32 value)
{
    ...
    default:
    {
        ...
        return (CPU_ALERT_TYPE)(alert_high | alert_low);
    }
    }
...
// There is a difference in operation depending on the TODO bitmap mode
#define write_vram8()       \
    address &= 0x1FFFE;     \
    if (address >= 0x18000) \
        address -= 0x8000;  \
...
#define write_memory(type)                                                                              \
    ...
    case 0x04:                                                                                          \
        ...
        break;                                                      /* 0x800 loops every 0x800: TODO */ \
                                                                                                        \
    case 0x05:                                                                                          \
        /* palatte RAM */                                                                               \
            ...
            if (parse_config_line(current_line, current_variable, current_value) != -1)
            {
                ...
                while (fgets(current_line, 256, config_file))
                {
                    if (parse_config_line(current_line, current_variable, current_value) != -1)
                    {
                        ...
                        // DBZLGCYGOKU2 protection avoidance
                        // Operation is possible if specified by a special object (currently unidentifiable) in EEPROM_V124
...
s32 load_gamepak_raw(char *name)
{
    ...
    if (FILE_CHECK_VALID(gamepak_file))
    {
        ...
        else
        {
            ...
            // When the current directory is changed in the file list
            // Specify the full path because the file becomes unreadable
...
s32 load_gamepak(char *name)
{
    ...
    {
        // Show progress bar
        set_cpu_clock(9);                        // Temporarily to 333MHz
        init_progress(5, msg[MSG_LOAD_ZIP_ROM]); // TODO error check
...
// Load the BIOS
// Return value: Normal up to MD5 0 / MD5 is different -2 / Cannot be read -1
...
#define dma_epilogue_iwram()                                                \
    ...
    {                                                                       \
        ...
        return_value = (CPU_ALERT_TYPE)(return_value | CPU_ALERT_SMC);      \
    }
...
CPU_ALERT_TYPE dma_transfer(DMA_TRANSFER_TYPE *dma)
{
    ...
    if (dma->irq)
    {
        ...
        return_value = (CPU_ALERT_TYPE)(return_value | CPU_ALERT_IRQ);
    }
...
void init_gamepak_buffer()
{
    ...
    if (psp_model == PSP_2000)
    {
        ...
        gamepak_rom_resume = (u8 *)malloc(4 * 1024 * 1024);
    }
    else
#endif
    {
...
/*--------------------------------------------------------
  State load
  input
    char *savestate_filename File name to load
    u32 slot_num             Used to determine slot number memory load
  return
    0 Load failure
    1 Successful load
--------------------------------------------------------*/
u32 load_state(char *savestate_filename, u32 slot_num)
{
    ...
    // TODO: Freezes when reading statesave file in different ROM
...
/*--------------------------------------------------------
  State save
  input
    char * savestate_filename Filename to load
    u16 * screen_capture Screen image
    u32 slot_num Slot No. Used to determine memory load 2
  return
    0 Save failed
    1 Save successful
--------------------------------------------------------*/
u32 save_state(char *savestate_filename, u16 *screen_capture, u32 slot_num)
{
    ...
    u64 current_time;
    pspTime current_time_fix; // For adjustment because the time function does not return the date
